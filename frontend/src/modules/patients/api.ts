...
export const patientsApi = {
  getPatients: async (): Promise<Patient[]> => {
    return http.get<Patient[]>("/api/patients")
  },

  getPatient: async (patientFhirId: string): Promise<Patient | null> => {
    try {
      return await http.get<Patient>(`/api/patients/${patientFhirId}`)
    } catch {
      return null
    }
  },
  ...
  getEncounters: async (patientFhirId: string): Promise<Encounter[]> => {
    return http.get<Encounter[]>(`/api/patients/${patientFhirId}/encounters`)
  },

  createEncounter: async (encounterData: Omit<Encounter, "fhir_id" | "created_at" | "status">): Promise<Encounter> => {
    return http.post<Encounter>(`/api/patients/${encounterData.patient_fhir_id}/encounters`, {
  ...
  getObservations: async (encounterFhirId: string): Promise<Observation[]> => {
    return http.get<Observation[]>(`/api/encounters/${encounterFhirId}/observations`)
  },

  getAllPatientObservations: async (patientFhirId: string): Promise<Observation[]> => {
    return http.get<Observation[]>(`/api/patients/${patientFhirId}/observations`)
  },

  createObservation: async (observationData: Omit<Observation, "fhir_id" | "created_at">): Promise<Observation> => {
    return http.post<Observation>(`/api/encounters/${observationData.encounter_fhir_id}/observations`, {
  ...
  getConditions: async (patientFhirId: string): Promise<Condition[]> => {
    return http.get<Condition[]>(`/api/patients/${patientFhirId}/conditions`)
  },

  createCondition: async (conditionData: Omit<Condition, "fhir_id" | "created_at">): Promise<Condition> => {
    return http.post<Condition>(`/api/patients/${conditionData.patient_fhir_id}/conditions`, {
  ...
  getDiagnosticReports: async (encounterFhirId: string): Promise<DiagnosticReport[]> => {
    return http.get<DiagnosticReport[]>(`/api/encounters/${encounterFhirId}/reports`)
  },

  createDiagnosticReport: async (reportData: Omit<DiagnosticReport, "fhir_id" | "created_at" | "status">): Promise<DiagnosticReport> => {
    return http.post<DiagnosticReport>(`/api/encounters/${reportData.encounter_fhir_id}/reports`, {
